...
class DIISCO:
    ...
    def _get_model_hypers(self) -> dict:
        ...
        w_length_scale_init = torch.tensor(
            self.hypers_init_vals[names.LENGTHSCALE_W]
        )
        w_length_scale = pyro.param(
            names.LENGTHSCALE_W,
            w_length_scale_init,
        ...
        while samples_so_far < n_samples:
            ...
            for i in range(self.n_cell_types):
                for j in range(self.n_cell_types):
                    ...
                    w_sample = regressor.sample(timepoints, n_samples=n_samples_per_latent)
            ...
            assert f_predict.shape == (n_samples_per_latent, self.n_cell_types, timepoints.shape[0])
    ...
    def _get_guide(self, guide_name: str, model: Callable) -> Callable:
        ...
        if guide_name == "MultivariateNormal":
            ...
            return guides.MultivariateNormalFactorized(
                self.n_cell_types, self.n_timepoints, self.prior_f_mean
            )
        else:
            raise ValueError(
